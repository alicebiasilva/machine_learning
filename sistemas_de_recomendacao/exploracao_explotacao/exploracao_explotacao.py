"""
Esse script tem como objetivo recomendar filmes de duas formas: 
- exploração (explorar filmes com base no comportamento anterior do usuário) e 
- explotação (recomendar filmes com base nos padrões de gosto de usuários semelhantes). 
Ele usa os dados de interações dos usuários e as características dos filmes para fazer as recomendações.


Esta classe herda a classe Recommender e é usada para recomendar itens com base em um modelo de 
exploitação, ou seja, priorizando os itens que já têm mais interações (cliques, sentimentos, etc.).

Além disso, cria a classe exploiter, que se baseia em explotar itens populares e sugerir mais 
os itens que os usuários similares já gostaram (com base em perfis de usuários semelhantes).

Fluxo completo do que o código faz
1. Prepara os dados (filmes e ratings) a partir dos arquivos CSV.
2. Cria um sistema de recomendação (classe Exploiter).
3. Calcula a popularidade dos filmes e recomenda os mais populares com get_popular_movies().
4. Explora os filmes com base nas interações passadas do usuário, usando recommend_by_exploration().
5. Exploita os filmes com base em perfis semelhantes, usando exploit_similar_profiles().

Saída do Script
1. Filmes populares: Lista de filmes mais populares, com base nos dados de cliques e sentimentos.
2. Recomendações de exploração: Filmes recomendados com base nas interações passadas de um usuário.
3. Recomendações de exploração com base em perfis semelhantes: base nas preferências de usuários 
com perfis similares ao do usuário atual.
"""
...
import sys
sys.path.append(r"C:\Users\alice\OneDrive\Área de Trabalho\machine_learning")

from sistemas_de_recomendacao.exploracao_explotacao.schemas import UserInteractions, UserProfile, Item
from sistemas_de_recomendacao.popularidade.recomendacao_popularidade import Recommender
